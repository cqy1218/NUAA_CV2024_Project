...
class Coeffs(nn.Module):
    ...
    def __init__(self, nin=16, nout=17, options=None):
        ...
        self.lb = int(options.luma_bins/2)  #8#params['luma_bins']
        ...
        self.local_features.append(ConvBlock(256, 128, 3, stride=1, batch_norm=bn))
        self.local_features.append(ConvBlock(128,128, stride=1, activation=None, use_bias=False))
#32 * self.cm * self.lb, 32 * self.cm * self.lb
        # predicton
    ...
    def forward(self, lowres_input):
        ...
        s = x.shape
        x = x.view(int(bs * self.G), int(self.nin * self.nout), self.lb, int(s[2]), int(s[3]))  # B x Coefs x Luma x Spatial x Spatial
        return x
...
class AttModule(nn.Module):
    def __init__(self, options):
        ...
        self.grid_channel = int(options.luma_bins*16*17/2)
