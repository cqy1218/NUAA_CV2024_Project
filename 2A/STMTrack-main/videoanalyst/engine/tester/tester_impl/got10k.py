...
class GOT10kTester(TesterBase):
    ...
    def test(self, ):
        ...
        for subset in self._hyper_params["subsets"]:
            ...
            # single worker
            if True:  # 修改：nr_devs == 1 -> True
                dev = all_devs[0]
                self._pipeline.set_device(dev)
                pipeline_tracker = PipelineTracker(tracker_name, self._pipeline)
                experiment.run(pipeline_tracker, verbose=self._hyper_params["verbose"])
            # multi-worker
            else:
                procs = []
                ...
                for p in procs:
                    p.join()
            # evalutate
            performance = experiment.report([tracker_name], plot_curves=True)  # 修改：False -> False
    ...
    def worker(self, dev_id, dev, subset, slicing_quantile):
        ...
        experiment = ExperimentGOT10k(root_dir,
                                      ...
                                      result_dir=result_dir,
                                      report_dir=report_dir)
        experiment.run(pipeline_tracker, slicing_quantile=slicing_quantile, verbose=self._hyper_params["verbose"])
